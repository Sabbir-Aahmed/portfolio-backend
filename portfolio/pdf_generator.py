...
class ResumePDFGenerator:
    ...
    def setup_custom_styles(self):
        ...
        if 'LinkHover' not in self.styles.byName:
            self.styles.add(ParagraphStyle(
                name='LinkHover',
                parent=self.styles['Normal'],
                fontSize=10,
                textColor=colors.HexColor('#D90429'),  # Darker red for hover
                spaceAfter=4,
                alignment=1
            ))
    ...
    def generate_resume_pdf(self, resume):
        ...
        contact_parts = []
        if resume.email:
            contact_parts.append(f'<a href="mailto:{resume.email}" color="{self.accent_color.hexval()}">{resume.email}</a>')
        if resume.phone:
        ...
        social_links = []
        link_style = f'color="{self.accent_color.hexval()}"'
        hover_style = f'color="{colors.HexColor("#D90429").hexval()}"'
        
        if resume.linkedin_url:
            social_links.append(f'<a href="{resume.linkedin_url}" {link_style}>LinkedIn</a>')
        if resume.github_url:
            social_links.append(f'<a href="{resume.github_url}" {link_style}>GitHub</a>')
        if resume.portfolio_url:
            social_links.append(f'<a href="{resume.portfolio_url}" {link_style}>Portfolio</a>')
        ...
        if resume.experiences.exists():
            ...
            for exp in resume.experiences.all():
                ...
                # Create a table for better alignment of company/position and dates
                exp_data = [
                    [
        ...
        footer_text = f"Generated on {resume.updated_at.strftime('%B %d, %Y')}"
